  ...
  return (
    ...
    >
      {menuItems.map((item) => (
        ...
        >
          <div
            className={`flex ${isMobileMenu ? 'p-[17px] bg-black/10 w-full border-b-[0.5px] border-black/20' : 'p-0 bg-transparent'}`}
          >
            {/*content*/}
            ...
            {item.isNew && (
              <span className="relative flex items-center justify-center ml-1 mt-0.5 bg-black overflow-hidden rounded-sm w-7 h-4">
                <span
                  className={`flex z-10 text-[10px] font-semibold text-white ${isMobileMenu ? 'mt-0.5' : 'mt-[1px]'}`}
                >
                  New
                </span>
                <motion.span
