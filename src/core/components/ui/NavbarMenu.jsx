...
const NavbarMenu = () => {
  return (
    <div className="flex gap-5 text-sm font-inter-tight">
      {menuItems.map((item) => (
        <Link
          key={item.name}
          to={item.path}
          className="flex items-center justify-center gap-1 hover:text-blue-500 relative"
        >
          <span>{item.name}</span>
          {item.icon && <span>{item.icon}</span>}

          {item.isNew && (
            <span className="relative text-[10px] flex items-center justify-center font-semibold text-white bg-black overflow-hidden rounded-sm min-w-[30px] min-h-[15px]">
              <span className="flex z-10 mt-0.5">New</span>
              <motion.span
